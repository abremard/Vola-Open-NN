...
Contributors :
    -
Version Control :
    1.0 - 07/06/2020 : division, annotate, test
"""
...
def annotate(stockNames, dayStart, dayEnd, benchmark=False, exportHTML=False):    
    """Results are saved as a benchmark file with profit, success, sizing for each day
        Users can also view and/or save graphs as HTML, containing renko candlesticks, long/shorts, stop losses...

    Args:
        stockNames (str[]): array of stock names to be simulated
        dayStart (str "%Y-%M-%d"): lower bound
        dayEnd (str "%Y-%M-%d"): upper bound
        benchmark (bool, optional): export results as benchmark CSV. Defaults to False.
        exportHTML (bool, optional): export daily screenshots as HTML. Defaults to False.
    """    
    nbDays = 0
    totalProfit = 0
    totalPositions = 0
...
# ----------------------------------------------------------------- Test
def test():
    """This function is internal to ANNOTATION.py, it is meant for debugging but also serves as unit test
    """
    dayStart = datetime.date.today() - datetime.timedelta(days = 5)
    dayEnd = datetime.date.today()
    stockNames = ["AMZN","AAPL","ABT","ACN","AMGN","ADBE","BA","CCEP","CMCSA","CSCO","CVX","DOW","FB","HD","INTC","JNJ","MO","NFLX"]
    annotate(stockNames, dayStart, dayEnd)
